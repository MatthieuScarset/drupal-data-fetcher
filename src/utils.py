...
def rename_taxonomy_columns(
    df: pd.DataFrame, col_name_prefix: str = "taxonomy_vocabulary_"
):
    """
    ...
    if df_vocabularies.empty:
        raise ValueError("Vocabulary DataFrame is empty. Cannot rename columns.")
...
def load_large_csv_to_pd(
    input_file: str, chunk_size: int = typer.Option(1000)
) -> pd.DataFrame:
    chunk_list = []
...
def get_last_commits(
    fetcher: Fetcher, project_id: str, namespace: str = "project"
) -> Optional[str]:
    """
...
def get_core_compatibility(
    fetcher: Fetcher, field_project_machine_name: str, version: str = "current"
) -> Optional[str]:
    """
    ...
    try:
        ...
        if matches:
            ...
            logger.warning(f"{field_project_machine_name}: no core compatibility found")
